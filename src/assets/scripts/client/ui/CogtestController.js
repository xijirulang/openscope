...
export default class CogtestController {
    constructor($element) {
        this._eventBus = EventBus;
        this.$element = $element;
        this.$dialog = null;
        this._currentStage = 'intro';
        this._data = {
            pvt: null,
            nback: null,
            stroop: null
    ...
    _setupHandlers() {
        ...
        this._onOpenRequestHandler = this.onOpenRequest.bind(this);
        ...
        this._onCogtestDoneHandler = this.closeDialog.bind(this);
    ...
    init() {
        this.$dialog = $(UI_COGTEST_MODAL_TEMPLATE);
        this.$dialog.find('.js-cogtestStartButton').on('click', this._onCogtestStartHandler);
        this.$dialog.find('.js-pvtResponseButton').on('mousedown', this._onPvtResponseHandler);
        this.$dialog.find('.js-nbackYesButton').on('click', this._onNBackYesHandler);
        this.$dialog.find('.js-nbackNoButton').on('click', this._onNBackNoHandler);
        this.$dialog.find('.js-stroopLeftButton').on('click', this._onStroopLeftHandler);
        this.$dialog.find('.js-stroopUpButton').on('click', this._onStroopUpHandler);
        this.$dialog.find('.js-stroopRightButton').on('click', this._onStroopRightHandler);
        this.$dialog.find('.js-cogtestExportButton').on('click', this._onCogtestExportHandler);
        this.$dialog.find('.js-cogtestRestartButton').on('click', this._onCogtestRestartHandler);
        this.$dialog.find('.js-cogtestDoneButton').on('click', this._onCogtestDoneHandler);
    ...
    enable() {
        ...
        this._eventBus.on(EVENT.COGTEST_OPEN, this._onOpenRequestHandler);
    ...
    disable() {
        ...
        this._eventBus.off(EVENT.COGTEST_OPEN, this._onOpenRequestHandler);
    ...
    goToStage(stageName) {
        ...
        this.$dialog.find('.cogtest-stage').addClass('cogtest-stageHidden');
        this.$dialog.find(`.cogtest-stage[data-stage="${stageName}"]`).removeClass('cogtest-stageHidden');
    ...
    initPVT() {
        ...
        this.$dialog.find('.js-pvtStatus').text('等待信号变红后点击按钮');
        this.$dialog.find('.js-pvtCounter').text(`第 ${this._pvtTrial + 1}/${PVT_TOTAL} 次`);
        this.$dialog.find('.js-pvtResponseButton')
    ...
    triggerPvtStart() {
        ...
        this.$dialog.find('.js-pvtStatus').text('请等待红色信号出现后立即点击');
        this.$dialog.find('.js-pvtCounter').text(`第 ${this._pvtTrial + 1}/${PVT_TOTAL} 次`);
        this.$dialog.find('.js-pvtResponseButton')
            .removeClass('cogtest-pvt-active')
            .text('等待信号...');
        ...
        this._timers.pvtStart = window.setTimeout(() => {
            ...
            this.$dialog.find('.js-pvtStatus').text('立即点击按钮！');
            this.$dialog.find('.js-pvtResponseButton')
    ...
    recordPvtResult(reactionTime, type) {
        this._pvtState = 'idle';
        this.$dialog.find('.js-pvtStatus').text(`本次结果：${type}`);
        this.$dialog.find('.js-pvtResponseButton')
            .removeClass('cogtest-pvt-active')
            .text('等待下一次...');
    ...
    runNBackStep() {
        ...
        this.$dialog.find('.js-nbackCounter').text(`第 ${this._nbackIndex + 1}/${NBACK_TOTAL} 项`);
        this.$dialog.find('.js-nbackStimulus').text(`${item.callsign} ｜ ${item.altitude}`);
        this.$dialog.find('.js-nbackPrompt').text(
    ...
    handleNBackResponse(answer) {
        if (this._currentStage !== '2back' || !this._nbackPlaying || !this._nbackAwaitingResponse) {
            return;
        }
    ...
    runStroopStep() {
        ...
        this.$dialog.find('.js-stroopCounter').text(`第 ${this._stroopTrial + 1}/${STROOP_TOTAL} 题`);
        this.$dialog.find('.js-stroopStimulus').text(`${stimulus.arrow} ${stimulus.word}`);
    ...
    handleStroopResponse(direction) {
        if (this._currentStage !== 'stroop' || !this._stroopPlaying || !this._stroopAwaitingResponse) {
            return;
        }
    ...
    renderResults() {
        const pvt = this._data.pvt || {};
        const nback = this._data.nback || {};
        const stroop = this._data.stroop || {};
        ...
        this.$dialog.find('.js-cogtestResults').html(html.join(''));
    }
    ...
    exportData() {
        ...
        try {
            ...
            this.$dialog.find('.js-cogtestResults').append('<p>导出失败，请检查浏览器下载权限。</p>');
            return;
        }
